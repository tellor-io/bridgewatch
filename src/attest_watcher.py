...
def main():
    ...
    Main function
    """
    parser = argparse.ArgumentParser(description="Monitor verifyOracleData calls to bridge contract")
    ...
    parser.add_argument("--output", default="attestations", help="Output file prefix")
    ...
    try:
        watcher = AttestWatcher(args.provider, args.bridge, args.output)
        
        if args.once:
            # run once
            state = watcher.run_monitoring_cycle(args.buffer)
            print(f"Scan complete. Found {state['total_calls_found']} total calls.")
